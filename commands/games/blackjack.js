...
const drawIntoDealer = () => {
    ...
    if(card.length === 2) { 
        (card[0] === 'J' || card[0] === 'Q' || card[0] === 'K') ? dealerHandValue += 10 :  dealerHandValue +=  Number(card[0])
...
// Action Buttons
const normalBlackjack = new ButtonBuilder().setLabel('Normal').setStyle(ButtonStyle.Primary).setCustomId('normal-button');
const highRollBlackjack = new ButtonBuilder().setLabel('HighRoll').setStyle(ButtonStyle.Success).setCustomId('highroll-button');

const lowBet = new ButtonBuilder().setLabel('150').setStyle(ButtonStyle.Secondary).setCustomId('low-button');
const midBet = new ButtonBuilder().setLabel('500').setStyle(ButtonStyle.Primary).setCustomId('mid-button');
const topBet = new ButtonBuilder().setLabel('1000').setStyle(ButtonStyle.Success).setCustomId('top-button');

const hitButton = new ButtonBuilder().setLabel('Hit').setStyle(ButtonStyle.Success).setCustomId('hit-button');
const standButton = new ButtonBuilder().setLabel('Stand').setStyle(ButtonStyle.Primary).setCustomId('stand-button');

const exitButton = new ButtonBuilder().setLabel('Exit').setStyle(ButtonStyle.Danger).setCustomId('exit-button');
        ...
        try {
            ...
            collector.on('collect', async (interaction) => {
                ...
                if(interaction.customId === 'normal-button') {
                    ...
                    interaction.editReply({ content: 'Place your bets...', components: [betsRow] });
                    ...
                    collector.on('collect', async (interaction) => {
                        ...
                        if(interaction.customId === 'mid-button') {
                            ...
                            interaction.editReply({ content: `${dealerHand}`, components: [] });
                        }
